...
// testKafkaIO builds a kafkaIO with just enough set for breakerState() —
// the only thing the /ready handler under test actually calls on it — so
// this test doesn't need a real Kafka broker.
func testKafkaIO() *kafkaIO {
	return &kafkaIO{breaker: reliability.NewCircuitBreaker(5, 15*time.Second)}
}
...
// {"ready": true} regardless of whether anomaly-detector could actually
// reach Postgres — its one genuinely-required dependency, since detection
// depends on the Postgres-backed device/sensor catalog. It checks both
// directions: ready against a real, reachable database, and not-ready
// (503) against a catalog whose pool can never connect.
func TestReadyEndpoint_ReflectsRealPostgresState(t *testing.T) {
	...
	defer cancel()
	realCat, err := newCatalog(ctx, dsn, 5)
	if err != nil {
		t.Skipf("no live Postgres reachable, skipping: %v", err)
	}
	defer realCat.close()

	registry := newForestRegistry()

	port := freePort(t)
	startHealthServer(port, realCat, testKafkaIO(), registry)
	...
	// report it. Built directly (bypassing newCatalog, which requires an
	// initial successful refresh) since the point here is only to exercise
	// the /ready handler's ping check, not catalog loading itself.
	...
	deadCat := &catalog{pool: deadPool}

	deadPort := freePort(t)
	startHealthServer(deadPort, deadCat, testKafkaIO(), registry)
